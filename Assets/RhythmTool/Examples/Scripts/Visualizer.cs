    ...
    public class Visualizer : MonoBehaviour
    {
        ...
        private void OnOnset(Onset onset)
        {
            ...
            foreach (Chroma chroma in chromaFeatures)
                CreateLine(onset.timestamp, -2 + (float)chroma.note * .1f, .2f, Color.blue, onset.strength / 7);
            ...
            if (chromaFeatures.Count == 0)
                CreateLine(onset.timestamp, -2 + (float)lastNote * .1f, .2f, Color.blue, onset.strength / 7);
        }

        private void OnSegment(Value segment)
        {
            //Instantiate a line to represent the segment.
            CreateLine(segment.timestamp, -3, 1, Color.green, segment.value / 7);
        }
